...
def tunnelface_segmentation(input_file):
    ...
    json_path = input_file.rsplit('.', 1)[0] + '_seg.json'
...
def calculate_and_save_images(img_path):
    ...
    parts = parts[1]
    output_folder = os.path.join(os.path.dirname(img_path),parts)
    ...
    for row in range(num_rows):
        for column in range(num_columns):
            ...
            new_file_name = f"{parts}_{column}_{row}_{num_columns}x{num_rows}.png"
    ...
    with open(txt_file_path, 'w') as txt_file:
        txt_file.write(f"File Name: {parts}\n")
...
def show_seg(img_path):
    image = cv2.imread(img_path)
    output_path = img_path.rsplit('.', 1)[0] + "_seg.png"

    TF_filepath = img_path.rsplit('.', 1)[0] + "_seg.json"
    with open(TF_filepath, 'r', encoding='utf-8') as f:
        data_tf = json.load(f)
